...
#'
#' @param NAM a sparseMatrix NAM generated by [innerNAM]
#' @param batch A factor containing batch information
...
#' Default NULL
#' @param ridge A numeric vector of ridge penalties to try. If NULL, a default
...
#' updating ridges. \cr
#' Default: 6
#' @param use.iter.ridge A logical controlling whether ridge weights should
#' be iteritively applied. The default is TRUE to preserve original behavior,
#' but FALSE is the correct way to try multiple ridges and only perform
#' the ridge regression once when kurtosis threshold is passed.\cr
#' Default: TRUE
#' 
...
residNAM <- function(NAM, 
    ...
    ncols_C <- 0
    if (!is.null(covs.mat)) {
        covs.mat <- scale(covs.mat)
        ncols_C <- ncols_C + ncol(covs.mat)
    }
    if (is.null(batch) || length(unique(batch)) == 1) {
        message('Only one unique batch supplied.')
        if (is.null(covs.mat)) {
            M <- Matrix::Diagonal(n = N)
        } else {
            M <- Matrix::Diagonal(n = N) - covs.mat %*% solve(crossprod(covs.mat, covs.mat), Matrix::t(covs.mat)) 
        }
        NAM_ <- M %*% NAM_ 
    } else {
        B <- model.matrix(~0+batch)
        # TODO: I don't know why this is being scaled
        B <- scale(B) 
        ...
        if(use.iter.ridge){
            for(ridge in ridges) {
                ...
                M <- Matrix::Diagonal(n = N) - C %*% solve(crossprod(C, C) + ridge * nrow(C) * L, Matrix::t(C))
                # TODO: fix this
                ## Why do you want to update this iteratively? 
                NAM_ <- M %*% NAM_
                kurtoses <- batchKurtosis(NAM_, batch)
                message("with ridge ", ridge, " median batch kurtosis = ", median(kurtoses))
...
#' 
#' @param data A cna data list.
#' @param n.steps Number of steps to take during the random walk.\cr
#' Default: NULL
#' @param min.steps minimum number of steps to take if n.steps is NULL.\cr
#' Default: 3
#' @param max.steps Maximum number of steps to take if n.steps is NULL.\cr
#' Default: 15
#' @param kurt.delta the minimum change 
#'
...
innerNAM <- function(data, 
    ...
    for (i in seq_len(max.steps)) {
        ...
        if (is.null(n.steps)) {
            if (prevmedkurt - medkurt < kurt.delta & i > min.steps) {
...
#' Default: FALSE
#' @param ... Additional parameters passed to [innerNAM].
#' @param force.recompute STUB. Not used currently. 
...
nam <- function(data, 
    ...
    #     if filter.samples is None:
    #         if covs is not None:
    #             filter.samples = ~np.any(np.isnan(covs), axis=1)
    #         else:
    #             filter.samples = np.repeat(True, data.N)
    #     else:
    #         filter.samples = _df_to_array(data, filter.samples)
    ...
    if (is.null(covs)) {
        ...
        ## Assumes numeric
        cov_mat <- dplyr::pull(data$samplem, dplyr::one_of(covs)) |> 
            as.numeric()
    }
    ...
    NAM <- innerNAM(data, n.steps = nsteps, ...)
    if (verbose) message('QCing NAM')
    res_qc_nam <- qcNAM(NAM, batch = batch_vec) 
    ...
    res_resid_nam <- residNAM(NAM, covs.mat = cov_mat, batch = batch_vec)
